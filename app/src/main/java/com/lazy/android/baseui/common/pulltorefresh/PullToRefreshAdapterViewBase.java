...
public abstract class PullToRefreshAdapterViewBase<T extends AbsListView> extends PullToRefreshBase<T> implements
	...
	public final void onScroll(final AbsListView view, final int firstVisibleItem, final int visibleItemCount,
		...
		//增加
		// 列表页提前加载 加载到本页第五条记录的时候就加载  有时候会因为网络返回的数据慢出现重复页面的情况，
		// 可以把请求回来的数据缓存到本地的数据库，去掉重复的记录
//		Log.i("chenglei", "firstVisibleItem= " + firstVisibleItem + "|visibleItemCount=" + visibleItemCount + "|totalItemCount=" + totalItemCount);
		if (tempfrist != firstVisibleItem) {
			tempfrist = firstVisibleItem;
			if (firstVisibleItem - 5  == totalItemCount - ConfigSystem.PAGE_SIZE ) {
